...
const AuthQuestions: React.FC<AuthQuestionsProps> = ({
  ...
  return (
    <div className="font-[] mt-16 md:mt-28">
          ...
          <div
            className={` mt-3  ${
              topicQuestion === "What do you plan on doing?"
                ...
                : "md:grid flex-col  max-md:space-y-4 flex grid-cols-3 md:gap-5"
            }`}
          >
            {topicQuestion === "What do you plan on doing?" ? (
              item.content.map((plan) => (
                <label
                  key={plan.contentId}
                  className={`flex items-center font-normal  px-4 justify-start h-[60px] md:h-[50px] w-full md:w-[150px] ${
